Commit - adds changes to working repository
...
Has a constructor that accepts a regular expression as a parameter. The regular expression is stored on the object for future reference.
Has a Connected getter that defines a constant for the class that is set to a value of 'connected'.
Has a connect method that takes a variable number of values. The method joins the values together into a string that is separated by a '-' character and attempts to match it with the regular expression given in the constructor. If it fails it throws an exception. Otherwise it passes back an object that has an id and result property, where the result is set to Phone.Connected.
